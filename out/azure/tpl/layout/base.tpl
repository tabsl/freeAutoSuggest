...
<body>
    ...
    <![endif]-->
	<script type="text/javascript" src="[{ $oViewConf->getBaseDir() }]out/azure/src/js/prodpop.js"></script>
	[{if $actCategory->oxcategories__oxfilter->value == 1}]	
	<script type="text/javascript" src="[{ $oViewConf->getBaseDir() }]out/azure/src/js/jquery.form.js"></script>
	...
	var options = { 
		...
		/*success:    function() { 
        alert('TEST erfolgreich: Funktion ausgeführt!'); 
    } */
	};
    ...
    $("#listcontent").html('<div style="text-align:center; margin-top:150px"><img src="[{ $oViewConf->getBaseDir() }]ajaxload.gif"></div>');
    return true; 
	}
	</script>
	[{/if}]	
	<script type="text/javascript" src="[{ $oViewConf->getBaseDir() }]out/azure/src/js/initall.js"></script>
